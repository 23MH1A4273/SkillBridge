...
function extractExperience(extensions = []) {
  return extensions.find((e) => /\d.*year|yr|experience/i.test(e)) || null;
}

function extractSalary(extensions = []) {
  return extensions.find((e) => /₹|\$|lpa|lakh|salary|per year|per month/i.test(e)) || null;
}
...
function normaliseJob(raw, queryStr) {
  ...
  const required =
    detected.length > 0 ? detected.slice(0, 6) : queryStr.split(/\s+/).filter(Boolean);

  const preferred =
    detected.length > 6 ? detected.slice(6, 12) : ['Problem Solving', 'Communication', 'Teamwork'];
  ...
  const directUrl =
    applyOptions[0]?.link || // e.g. "Apply on LinkedIn", "Apply on Naukri"
...
async function main() {
  ...
  try {
    ...
    console.log(
      `🔗  View at: https://console.firebase.google.com/project/skillbridge-84575/firestore`
    );
    console.log('═══════════════════════════════════════════════\n');
  } catch (err) {
